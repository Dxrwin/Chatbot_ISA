...
class PayableRequest(BaseModel):
    creditLineID: str | None = None
    principal: float | None = None
    time: int | None = None
    disbursementMethod: str | None = None
    initialFee: float | None = None
    paymentFrequency: int | None = None
...
async def webhook_product_lines(parent_id: str):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            logger.info(f"parametros recibidos: parent_id={parent_id}")
            logger.info("Iniciando proceso de obtención de token y consulta de API externa.")
            # 1. Obtener token
            ...
            # 2. Consultar API externa protegida
            response = await client.get(API_URL, headers=headers)
            response.raise_for_status()
            data = response.json()
            lines = data.get("data", {}).get("lines", [])
            logger.info(f"Datos recibidos de la API: {lines}")
            
            # Buscar coincidencias en el JSON externo
        for line in lines:
            
            if line.get("parentId") == parent_id:
                
                logger.info(f"Línea encontrada: {line}")
                logger.info(f"Nombre de la línea: {line.get('name')}")
        ...
        logger.error(f"Error en API externa: {e.response.text}")
...
async def create_payable(client_id: str, payload: PayableRequest):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            logger.info(f"Parámetros recibidos: client_id={client_id},\n payload desde chat ={payload} \n")
            logger.info(f"Payload original: {payload} \n")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if status_code == 201:
                        ...
                        return {"status": "error", "codigo_status": status_code, "detalle": response.json()}
...
# Endpoint para calcular financiamiento
...
async def calcular_financiamiento(payload: dict):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            KUENTA_URL = f"https://demo-api.kuenta.co/v1/products/{linea_producto}"
...
async def obtener_estado(debtor_id:str,request: Request):
    ...
    url = f"https://demo-api.kuenta.co/v1/payable/{creditid}/installment/0/order/list/{orderid}"
    intentos = 3
